...
import { LogOut, User as UserIcon, Search, Bell, MessageSquare } from "lucide-react";
  ...
  return (
    <nav className="fixed top-0 w-full h-16 z-50 flex items-center justify-between px-6 border-b border-white/[0.04] bg-[#171718]">
      ...
      <div className="flex items-center gap-2 w-64 flex-shrink-0">
        <h1 className="text-xl font-bold bg-clip-text text-transparent bg-gradient-to-r from-violet-400 to-indigo-400 cursor-pointer" onClick={() => navigate("/")}>
          SocialForum
        </h1>
      </div>
      ...
      <div className="hidden md:flex flex-1 max-w-xl mx-4">
        <div className="relative w-full group">
          <Search size={18} className="absolute left-3 top-1/2 -translate-y-1/2 text-white/30 group-focus-within:text-indigo-400 transition-colors" />
          <Input 
