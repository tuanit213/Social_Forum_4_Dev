...
export default function Globe({
    ...
    // Stabilize object props với useMemo để tránh re-render vô ích
    const stableDots = useMemo(() => dots, [
        dots.color, dots.size, dots.density, dots.allDots
    ]);
    const stableMarkerConfig = useMemo(() => markerConfig, [
        markerConfig.color, markerConfig.size,
        // eslint-disable-next-line react-hooks/exhaustive-deps
        JSON.stringify(markerConfig.markers)
    ]);

    const dotColor = stableDots.color;
    const dotSize = stableDots.size;
    const density = stableDots.density;
    const allDots = stableDots.allDots;
    const gridWidth = 1;
    ...
    useEffect(() => {
        ...
        // Giữ canvas luôn visible, chỉ fade in sau khi load xong
        ...
        const loadWorldData = async () => {
            try {
                ...
                // Fade in mượt thay vì bật tắt đột ngột
        ...
        // Chỉ dùng các giá trị primitive ổn định
        // Object như dots/markerConfig đã được memo ở trên
        speed,
        smoothing,
        fill,
        fillColor,
